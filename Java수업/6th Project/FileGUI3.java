...
// JPanel
public class FileGUI3 extends JPanel implements ActionListener{
	...
	public FileGUI3() {
		...
		model = new DefaultListModel<>();// 모델이 있어야 값을 추가/삭제 가능
		...
		// model을 통해서 선택하지 않고 줄 번호를 통해 지울수 있다.		
		
		JScrollPane sp = new JScrollPane(list);
		btnSave = new JButton("저장");
		btnLoad = new JButton("불러오기");
		btnDel = new JButton("지우기");
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==btnSave) {// 이벤트가 일어난 객체의 getText()
		...
		}else if(e.getSource()==btnDel) {// 선택하고 지우기
			//list.getSelectedIndex();// 선택한 줄번호
			...
			String name = tfName.getText();
			String age = tfAge.getText();
			String val = idx+"/"+name+"/"+age;
			model.add(list.getSelectedIndex(), val);
			model.remove(list.getSelectedIndex()+1);
			
			saveFile();
			
	// model.add(번호, 내용)의 경우 추가할때 같은 번호가 있다면 그 번호에 추가되고
	// 원래 있던 줄의 내용은 다음줄로 밀려서 내려감.
		}
